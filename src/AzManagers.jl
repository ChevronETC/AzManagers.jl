...
function Distributed.create_worker(manager::AzManager, wconfig)
    ...
    try
        ...
        try
            ...
            Distributed.kill(manager, w.id, wconfig)
        finally
            rethrow(ex)
        end
    end
...
function azure_worker_start(out::IO, cookie::AbstractString=readline(stdin); close_stdin::Bool=true, stderr_to_stdout::Bool=true)
    ...
    errormonitor(@async while isopen(sock)
        ...
        #     break
        # end
        @info "cookie_from_master=$cookie_from_master, length(cookie_from_master)=$(length(cookie_from_master)), master_ip=$ipaddr, worker_ip=$(getipaddr()), worker_port=$(Distributed.LPROC.bind_port), __port=$__port"
