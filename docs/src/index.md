...
addprocs(AzManager(), "myscaleset", 5)
```
...
```julia
using AzManagers, Distributed
addprocs(AzManager(), "myscaleset", 5; waitfor=true)
```
...
Subsequently, `addprocs(AzManager(), "myscaleset", 5)` will query the json file for the VM template.  One can
...
detached_job = @detachat myvm begin
    ...
    addprocs(AzManager(), "myscaleset", 5; session=variablebundle(:session))
    for pid in workers()
...
Pkg.add("AzManagers")
Pkg.add("Jets")
addprocs(AzManager(), "cbox16",2;customenv=true)
```
