...
@testset "environment, addprocs" begin
    ...
    addprocs(AzManager(), templatename, 1; waitfor=false, group=group, session=session, customenv=true)
...
@testset "tags, addprocs" begin
    ...
    addprocs(AzManager(), template, 1; waitfor=false, group=group, session=session)
...
@testset "AzManagers, physical_hostname" begin
    ...
    addprocs(AzManager(), template, 2; waitfor=false, group=group, session=session)
