...
jinja_out = render(env, "cofii-base.json.j2"; subscriptionid=subscriptionid, resourcegroup=resourcegroup, clientId=credentials["clientId"], clientSecret=credentials["clientSecret"], tenantId=credentials["tenantId"], julia_version=julia_version)

write(stdout, ">>> $jinja_out <<<")
